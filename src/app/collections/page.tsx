...
export default function Collections({ params, searchParams }: CollectionsProps) {
  ...
  const { data: collection = [], isLoading } = useQuery({
    queryKey: ["collectionData", selectedInterval, orderConfig.orderBy, orderConfig.orderDirection],
    queryFn: () =>
  ...
  return (
    <>
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        ...
        >
          <section className="flex flex-col justify-between sm:flex-row gap-4 sm:gap-6 lg:gap-8 mb-4 sm:mb-6 lg:mb-7">
            {detail ? (
                ...
                <TabsList className="h-12 text-neutral50 p-1 border border-neutral400 rounded-xl gap-1 whitespace-nowrap">
                  {intervals.map((interval) => (
                    <TabsTrigger
                      key={interval}
                      value={interval}
                      className="min-w-[59px] h-10 font-semibold text-[15px] rounded-lg border-hidden"
                    >
                      {interval}
                    </TabsTrigger>
                  ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
              ...
              <div className="flex justify-center w-[100px] border border-neutral400 p-1 rounded-xl h-12">
                ...
                >
                  <Image src="/collections/hashtag.png" alt="grid view" width={20} height={20} />
                </button>
                <button
                  className={`w-11 h-10 rounded-lg p-3 ${viewType === "list" ? "bg-white4" : ""}`}
                  onClick={() => setViewType("list")}
                >
                  <Image src="/collections/burger.png" alt="list view" width={20} height={20} />
                </button>
              </div>
            </div>
          </section>
          ...
          {detail && (
            <section className={`flex w-full ${active ? "gap-6 lg:gap-10" : "gap-0"} pt-4 sm:pt-6 lg:pt-7`}>
              <div className={`${active ? "w-full sm:w-[280px] opacity-100" : "w-0 opacity-0"} transition-all duration-300`}>
                <CollectionSideBar />
              </div>
                ...
                <div>
                  {viewType === "grid" ? (
                    ...
                    <div className="overflow-hidden">
                      ...
                      <ScrollArea className="h-[500px] sm:h-[600px] lg:h-[754px] border-t-2 border-neutral500">
